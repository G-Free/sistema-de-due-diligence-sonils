...
const AssessmentTemplateBuilder: React.FC<ModuleChangeProps> = ({ onModuleChange, setIsFormDirty }) => {
  ...
  const totalWeight = useMemo(() => {
    // FIX: Operator '+' cannot be applied to types 'unknown' and 'number'. Changed to iterate with Object.keys for consistent and safer type inference.
    return Object.keys(matrix[activeTab]).reduce((sum, key) => sum + matrix[activeTab][key].weight, 0);
  }, [matrix, activeTab]);
  ...
  return (
    ...
    <div className="bg-card p-6 rounded-lg shadow-sm animate-fade-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => onModuleChange('menu-dashboard')} className="bg-danger/10 hover:bg-danger/20 text-danger font-semibold py-2 px-4 rounded-lg transition-colors text-sm">
            Sair sem Salvar
        </button>
      </div>
            ...
            <div key={criterionName} className="bg-background border border-border rounded-lg transition-all duration-300">
              ...
              {openAccordion === criterionName && (
                <div className="p-4 border-t border-border animate-fade-in space-y-3">
                    {Object.entries(data.items).map(([key, question]) => (
