...
// Changed Icons to icons for consistent casing
...
const pageTitles: Record<string, string> = {
    ...
    "settings": "Configurações do Systema"
};
...
}> = ({ user, onLogout, activeModule, onModuleChange, pageTitle, notifications, onMarkAllAsRead }) => {
    ...
    return (
        <header className="h-20 flex items-center justify-between px-8 shrink-0 bg-card border-b-4 border-secondary shadow-md">
             <div className="flex items-center gap-6">
                <div className="h-12 w-32 bg-primary flex items-center justify-center text-white font-bold rounded">SONILS</div>
            ...
            <div className="flex items-center space-x-6">
                ...
                <div className="relative" ref={userMenuRef}>
                    <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center space-x-3 focus:outline-none">
                        <div className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center font-bold text-primary">{user.name.charAt(0)}</div>
                    ...
                    {showUserMenu && (
                        <div className="absolute right-0 mt-3 w-56 bg-card rounded-xl shadow-lg border border-border animate-fade-in-up origin-top-right z-10">
                           <div className="p-2">
                                <button onClick={() => handleUserMenuClick('user-management')} className="flex items-center w-full px-3 py-2 text-sm rounded-md text-text-secondary hover:bg-background hover:text-primary">
                                    <AdminIcon className="w-4 h-4 mr-3" />
                                    <span>Administração</span>
                                </button>
                                <div className="my-1 h-px bg-border"></div>
                                <button onClick={onLogout} className="flex items-center w-full px-3 py-2 text-sm rounded-md text-text-secondary hover:bg-background hover:text-primary">
