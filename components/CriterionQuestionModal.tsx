...
const CriterionQuestionModal: React.FC<CriterionQuestionModalProps> = ({ isOpen, onClose, onSave, config }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (config.mode === 'addCriterion' || config.mode === 'editCriterion') {
        ...
        // FIX: The weight state is already in decimal form (e.g., 0.1 for 10%).
        onSave({ name, weight });
    } else {
        if (!text.trim()) {
          ...
          {(config.mode === 'addCriterion' || config.mode === 'editCriterion') && (
            <>
              ...
              <div>
                ...
                {/* FIX: Convert percentage input value to decimal for state to fix UI bug. */}
                <input type="number" step="1" id="criterionWeight" value={weight * 100} onChange={e => setWeight(parseFloat(e.target.value) / 100)} required className="w-full bg-background border border-border rounded-lg p-2 text-text-main focus:ring-2 focus:ring-secondary focus:outline-none" />
              </div>
            </>
          )}
