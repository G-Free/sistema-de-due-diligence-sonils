...
const navItems = [
  ...
  { id: 'risk-assessment', label: 'Avaliação de Risco', icon: RiskAssessmentIcon },
  ...
  <li>
    <a
      ...
      className={`flex items-center p-2.5 rounded-lg transition-colors group ${
        isActive ? 'bg-primary/10 text-primary font-semibold' : 'text-text-secondary hover:bg-gray-100 hover:text-text-main'
      }`}
    >
      <Icon className={`w-5 h-5 mr-3 shrink-0 ${isActive ? 'text-primary' : 'text-text-secondary group-hover:text-text-main'}`} />
...
export const Sidebar: React.FC<SidebarProps> = ({ activeModule, onModuleChange, isOpen, setIsOpen, onLogout, user }) => {
  ...
  return (
    <>
        ...
        ></div>
        <aside className={`absolute lg:relative z-30 w-64 bg-card text-text-main h-full flex flex-col shrink-0 transition-transform transform border-r border-border ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
            <div className="h-20 flex items-center justify-center px-4 border-b border-border shrink-0">
                <img src="https://www.sonils.co.ao/images/logotipo.png" alt="SONILS Logo" className="h-8"/>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto p-4">
                ...
                <h3 className="px-2.5 text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">Sistema</h3>
                 <ul className="space-y-1.5">
            ...
            <div className="p-4 border-t border-border">
                <div className="p-2 rounded-lg hover:bg-background">
                 ...
                 <ul className="space-y-1 mt-2">
                    <NavItem id="admin" label="Administração" icon={AdminIcon} isActive={activeModule === 'admin'} onClick={handleNavigation}/>
                    <li>
                        <a href="#" onClick={(e) => { e.preventDefault(); onLogout();}} className="flex items-center p-2.5 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-main group">
